...
//	cout << "creating sig " << name << " sz " << size << " of type " << sort << endl;
	assert(name != "");
...
Inst* Inst::apply_children(InstL&args) {
	...
	case Op: {
		...
		assert(op != 0);
		return OpInst::create(op->get_op(), args, m_size);
	}
		break;
	case Ex: {
...
// 						}
	...
	// 		}
		...
		// 						ch_bit = vel_concat.front();
		// 					}else
							{
								ch_bit = OpInst::create(OpInst::Concat, vel_concat, 0, true);
							}
