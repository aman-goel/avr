...
#################################################### */
	wire enc_prop = (shiftreg[9:0] == 0) || (!(shiftreg[9:0]==0));
	assert property ( enc_prop );
...
######################################################################
*/
// Following property type not supported by AVR (due to clock delay)
...
###################################### */
	wire dec_prop = !(plain[7:0]==8'd255);
	assert property ( dec_prop );
